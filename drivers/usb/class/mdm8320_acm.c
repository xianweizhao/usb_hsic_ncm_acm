...
extern int modem8320_is_cp_wakeup_ap(void);
extern int mc_prepare_resume(int ms_time);
extern int mc_is_normal(void);
extern int mc_interface_count(int used);
...
#define ACM_DEFAULT_WAKE_TIME (3*HZ)
#define ACM_SUSPEND_UNLOCK_DELAY (2*HZ)
...
static void acm_post_resume_work(struct work_struct *work)
{
	...
	if (!ACM_CONNECTED(acm))
		return;
	dev = &acm->dev->dev;
	if (parent_acm->skip_hostwakeup && modem8320_is_cp_wakeup_ap) {
		dev_info(dev,
			"post resume host skip=%d, rpm_stat=%d",
			parent_acm->skip_hostwakeup, 
...
static int acm_read_done(struct acm_ch *ch,struct acm_rb *buf)
{
...
static int acm_write_start(struct acm *acm, int wbn)
{
	...
	/* 
	 * why initiated_resume and get interface async susp_count is not zero?
	 *
	 */
	if (acm->susp_count) {
		if (!acm->delayed_wb)
...
static void acm_read_bulk(struct urb *urb)
{
	...
	/* if urb count eq buffs, may not call urb_task */
	if (likely(!acm->susp_count))
		tasklet_schedule(&acm->urb_task);
}
...
static int acm_open(struct acm_ch *ch)
{
	...
	if(!mc_is_normal()){
		printk("%s:mc is not normal modem\n",__func__);
		return -ENODEV;
	}
	...
	/*
	 * why put interface so early? rcv_urb my not enque before suspend ?  
	 * ans: suspend will kill urb ,so not care.
	 * another problem:  mutex is locked  before put interface, but it maybe call suspend that it need mutex_lock??  
	 */ 
	usb_autopm_put_interface(acm->control); 
...
static void acm_close(struct acm_ch *ch)
{
	...
	acm_down(acm, 1);//to pass cp reset pressure test
	mutex_unlock(&open_mutex);	
	printk("%s: %d open count=%d\n", __func__, acm->cid, acm->count);
}
...
// LSI 
	...
	/* 
	 * here need porting, the last interface probe enbale echi auto_suspend.
	 */

	/******porting ***/
	mc_interface_count(1);

	acm_ch = acm_channel[acm->cid];

	if(acm_ch->notify){
		acm_ch->notify(acm_ch->priv, ACM_PROBE);
	}
...
static int acm_write_avail(struct acm_ch *ch)
{
	int i, n;
	unsigned long flags;
	struct acm *acm = acm_table[ch->cid];
...
static int acm_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if (message.event & PM_EVENT_AUTO) {
		...
		b = acm->processing + acm->transmitting;  //processing use ++ but not -- only set 0 how understand?
...
#define CYIT_MAIN_ACM_INFO(x) \
		USB_DEVICE_AND_INTERFACE_INFO(0x231e, x, \
...
static const struct usb_device_id acm_ids[] = {
	...
	{ CYIT_MAIN_ACM_INFO(0x6304),},/*cyit acm main hsic device*/
	{ }
...
struct acm_ch *acm_register(unsigned int cid, void (**low_notify)(struct acm_ch *),  int (*rxcb)(void * priv, struct acm_rb *buf), void(*wrcb)(void*), int (*notify)(void * priv,enum acm_state type), void *priv)
{
	if(cid < ACM_INTERFACES)
	{
		*low_notify = acm_kick;
		...
		acm_channel[cid]->write_avail = acm_write_avail;
		acm_channel[cid]->read_done = acm_read_done;
