...
#include "../../ipc_c8320/modem_v2/c8320_modem_ctl.h"
#endif
...
struct timer_list sc3_usbdev_timer;
int timer_sc3_init = 0;
...
extern int cpufreq_get_support_level_range(int* pMax, int* pMin);
extern int devfreq_get_support_level_range(int* pMax, int* pMin);
static struct pm_qos_request gadget_mif_qos;
static struct pm_qos_request gadget_int_qos;
static struct pm_qos_request gadget_cpumin_qos;
static spinlock_t gadget_freq_lock;
#define LOCK_TIMER_INTERVAL 5000 /*ms*/
static int s3c_udc_support_lock_freq(struct s3c_udc *dev)
{
	return dev->gadget.is_lock_cpufreq;
}
static void s3c_udc_lock_freq_work(struct work_struct *data)
{
	struct s3c_udc *dev = container_of(data, struct s3c_udc, lock_work);
	int max = 0, min = 0;
	//spin_lock(&gadget_freq_lock);
	if(!dev->is_locked){
		devfreq_get_support_level_range(&max, &min);
		pm_qos_update_request(&gadget_mif_qos,max);
		//pm_qos_add_request(&gadget_int_qos,PM_QOS_DEVICE_THROUGHPUT,200000);
		cpufreq_get_support_level_range(&max, &min);
		pm_qos_update_request(&gadget_cpumin_qos,max);
		dev->is_locked = 1;
	}
	//spin_unlock(&gadget_freq_lock);
	/*reset timer*/
	del_timer_sync(&dev->freq_lock_timer);
	mod_timer(&dev->freq_lock_timer,jiffies+msecs_to_jiffies(LOCK_TIMER_INTERVAL));
}
static s3c_udc_lock_freq(struct s3c_udc *dev)
{
	schedule_work(&dev->lock_work);
}
static void s3c_udc_unlock_freq_work(struct work_struct *data)
{
	struct s3c_udc *dev = container_of(data, struct s3c_udc, unlock_work);
	int max = 0, min = 0;
	/*this will run in a timer, so may be sho*/
	if(dev->is_locked){
		//spin_lock(&gadget_freq_lock);
		devfreq_get_support_level_range(&max, &min);
		pm_qos_update_request(&gadget_mif_qos,min);
		//pm_qos_remove_request(&gadget_int_qos);
		cpufreq_get_support_level_range(&max, &min);
		pm_qos_update_request(&gadget_cpumin_qos,min);
		dev->is_locked = 0;
		//spin_unlock(&gadget_freq_lock);
	}

}
static void s3c_udc_timer_func(unsigned long param)
{
	struct s3c_udc *dev = (struct s3c_udc *)param;
	schedule_work(&dev->unlock_work);
}
#endif
...
static struct s3c_udc *g_udc_dev = NULL;

int s3c_vbus_enable_ge(struct s3c_udc *dev, int is_active)
{
	unsigned long flags;
...
{
	if (!g_udc_dev) {
		printk("No udc init return\n");
		return 0;
	}
	if (onoff) {
		s3c_vbus_enable_ge(g_udc_dev, 1);
	} else  {
		s3c_vbus_enable_ge(g_udc_dev, 0);
	}
	return 1;
}
			 ...
			 struct usb_gadget_driver *driver)
{
	...
	if (is_nonswitch()
#if defined(CONFIG_USB_GADGET_SWITCH) || defined(CONFIG_USB_EXYNOS_SWITCH)
		||(startedcallcount && !dev->udc_enabled)
#endif
		)
		udc_enable(dev);
	else {
...
static int s3c_udc_probe(struct platform_device *pdev)
{
...
#if CONFIG_USB_GADGET_LOCK_FREQ
	{
		...
		dev->is_locked = 0;
		/*init timer*/
		init_timer(&dev->freq_lock_timer);
		dev->freq_lock_timer.function = s3c_udc_timer_func;
		dev->freq_lock_timer.data = (unsigned long) dev;
		INIT_WORK(&dev->lock_work, s3c_udc_lock_freq_work);
		INIT_WORK(&dev->unlock_work, s3c_udc_unlock_freq_work);
		//spin_lock_init(&gadget_freq_lock);
		devfreq_get_support_level_range(&max, &min);
		pm_qos_add_request(&gadget_mif_qos,PM_QOS_BUS_THROUGHPUT,min);
		//pm_qos_add_request(&gadget_int_qos,PM_QOS_DEVICE_THROUGHPUT,200000);
		cpufreq_get_support_level_range(&max, &min);
		pm_qos_add_request(&gadget_cpumin_qos,PM_QOS_CPU_FREQ_MIN,min);
	}
...
#if CONFIG_USB_GADGET_LOCK_FREQ
	pm_qos_remove_request(&gadget_mif_qos);
	//pm_qos_remove_request(&gadget_int_qos);
	pm_qos_remove_request(&gadget_cpumin_qos);
#endif
	the_controller = 0;
...
void sc3_set_phy2dev(void)
{
	writel(0, EXYNOS_USB_CFG_1);
}
static void sc3_usbdev_timer_func(unsigned long data)
{	
#ifdef CONFIG_C8320_HSIC_NEW
	if (c8320_modem_ctl_ex.debugmask & FORBID_SWITCH_USBDEV){
		printk("------------------------------%s:not set dev mode\n",__func__);
		return;
	};
	printk("------------------------------%s:set dev mode\n",__func__);
#endif
	sc3_set_phy2dev();
}
...
{
	int ret;
	init_timer(&sc3_usbdev_timer);
	timer_sc3_init = 1;
	sc3_usbdev_timer.function = sc3_usbdev_timer_func;
