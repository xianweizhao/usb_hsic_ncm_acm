...
#if defined(CONFIG_BATTERY_SAMSUNG) || defined(CONFIG_BATTERY_CW2015)
extern void cw2015_usb_det();
u32 cable_connected;
...
int s3c_udc_is_adc_cable()
{
	...
	for (i=0;i<10;i++)
	{
		val = __raw_readl(dev->regs + S3C_UDC_OTG_HPRT);
		//printk("HPRT%d:0x%x\n", i, val);
		if ((val & 0xc00) == 0xc00)
		{
			ac++;
...
static void complete_rx(struct s3c_udc *dev, u8 ep_num)
{
	...
	//printk("%s,%d,%d,%d,%d\n",__FUNCTION__,__LINE__,ep_num,xfer_length,s3c_udc_support_lock_freq(dev));
	if(ep_num && xfer_length>0 && s3c_udc_support_lock_freq(dev))
		s3c_udc_lock_freq(dev);
#endif
