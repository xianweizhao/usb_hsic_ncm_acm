...
struct io_device *get_iod_with_index(struct modem_shared *msd,
					unsigned index);
...
static inline struct io_device *link_get_iod_with_idex(struct link_device *ld, 
								unsigned index)
{
	struct io_device *iod = get_iod_with_index(ld->msd, index);
	return (iod && IS_CONNECTED(iod, ld)) ? iod : NULL;
}
